...
class QuizManager {
    ...
    private var _totalCorretAnswers = 0
    ...
    var totalCorrectAnswers: Int {
        return _totalCorretAnswers
    }
    
    func refeshQuiz() {
        let randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[randomIndex]
        quiz = Quiz(questions: quizData.question, options: quizData.options, correctAnswer: quizData.correctAnswer)
    }
    
    func validateAnswer(index: Int) {
        _totalAnswers += 1
        if quiz.validateOption(index) {
            _totalCorretAnswers += 1
        }
    }
}
